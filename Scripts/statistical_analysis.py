...
def distance(stat_file, distance_folder, feature_file):
    ...
    for feat, id, type, title, file_name in zip(feature, dist_id, sRNA_type, titles, file_names):
        if type != 'na':
            try:
                data = df[id][(df['feature'] == feat) & (df['sRNA_type'] == type)]
                plot = plt.hist(data, bins=range(-100, 100, 1))
                plt.xlabel('bp', size = 22)
                plt.xticks(size = 14)
                plt.yticks(size = 14)
                plt.title(title, size = 24)
                plt.savefig(os.path.join(distance_folder, f"{file_name}.pdf"), bbox_inches='tight')
                plt.close()
            except:
